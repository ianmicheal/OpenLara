...
#define MIN(a, b) ( (a)<(b)? (a):(b) )
			...
			switch (bpp) {
				...
				case 8: {
					...
					for (y=0; y<h; y += 2) {
						yout = y;
						for (x=0; x<w; x++) {
							vtex[TWIDOUT((yout&mask)/2, x&mask) + (x/min + yout/min)*min*min/2] = pixels[y*w+x] | (pixels[(y+1)*w+x]<<8);
						}
					}
				}
				break;
		...
		void upload_vram(uint8 *out, const uint8 *in, uint32 w, uint32 h) {
			uint8 *dst = out;
			uint32 *s = (uint32 *)in;
			uint32 *d = (uint32 *)(void *)(0xe0000000 | (((unsigned long)dst) & 0x03ffffc0));
...
#define COPY8888TO16(n) do {	\
	...
	*/
	...
	void setCullMode(int rsMask) {
		...
		switch (rsMask) {
			case RS_CULL_BACK  : 
				CullMode = PVR_CULLING_CW;  
				break;
			case RS_CULL_FRONT : 
				CullMode = PVR_CULLING_CCW;
				break;
            default: 
				CullMode = PVR_CULLING_NONE;
		}
	}
	...
	uint32 apply_lighting(const Vertex &vertex)
	{
		...
		uint8 R, G, B, A;
		R = clamp(int(result.x * argb[0]), 128, 255);
		G = clamp(int(result.y * argb[1]), 128, 255);
		B = clamp(int(result.z * argb[2]), 128, 255);
		A = argb[3];
		/* RGB might be wrong */
		//R = (R==0)? 100: R;
		//G = (G==0)? 100: G;
