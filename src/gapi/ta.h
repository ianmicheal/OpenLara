			...
			switch (bpp) 
			{
				...
				case 8: 
				{
					...
					for (y=0; y<h; y += 2) 
					{
						...
						for (x=0; x<w; x++) 
						{
							vtex[TWIDOUT((yout&mask)/2, x&mask) + (x/min + yout/min)*min*min/2] = 
...
#define COPY8888TO16(n) do { tmp = ARGB1555(s[0],s[1],s[2],s[3]); tmp |= ARGB1555(s[4],s[5],s[6],s[7]) << 16; d[n] = tmp; s += 8; } while(0)
