...
void joyUpdate() 
{
	...
	for(int i = 0; i < 4; i++, pad++) 
	{
		if( pad->func & MAPLE_FUNC_CONTROLLER && JoyCnt < 2) 
		{
			...
			Input::setJoyPos(JoyCnt, jkL, vec2(float(pad->cond.controller.joyx) , float(pad->cond.controller.joyy )) / 128.0f - 1.0f);
			Input::setJoyPos(JoyCnt, jkR, vec2(float(pad->cond.controller.joyx2), float(pad->cond.controller.joyy2)) / 128.0f - 1.0f);
